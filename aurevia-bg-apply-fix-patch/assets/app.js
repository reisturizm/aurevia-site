...
/* ===== Background hard fix patch ===== */
function applyBackgroundFromSettings(st){
  try{
    st = st || getSettings();
    const pageId = (document.body && document.body.getAttribute("data-page-id")) || "";
    const bgMap = {
    ...
    const bg = bgMap[pageId] || "";
    document.body.style.backgroundColor = "#0b0906";
    if(bg){
      document.body.style.backgroundImage = `linear-gradient(180deg, rgba(0,0,0,.72), rgba(0,0,0,.82)), url("${bg}")`;
  ...
  }catch(e){ console.warn("applyBackgroundFromSettings", e); }
}

(function(){
  const originalSync = (typeof syncGlobalText === "function") ? syncGlobalText : null;
  window.syncGlobalText = function(){
    try { if (typeof originalSync === "function") originalSync(); } catch(e){ console.warn("syncGlobalText original", e); }
    applyBackgroundFromSettings(getSettings());
  };

  const originalSave = window.saveSettingsFields;
  if(typeof originalSave === "function"){
    window.saveSettingsFields = async function(fieldsCsv, feedbackId){
      await originalSave(fieldsCsv, feedbackId);
      try{
        const st = getSettings();
        applyBackgroundFromSettings(st);
      }catch(e){ console.warn("saveSettingsFields bg patch", e); }
    };
  }

  document.addEventListener("DOMContentLoaded", ()=>{
    setTimeout(()=>applyBackgroundFromSettings(getSettings()), 50);
    setTimeout(()=>applyBackgroundFromSettings(getSettings()), 400);
  });
  window.addEventListener("pageshow", ()=>applyBackgroundFromSettings(getSettings()));
})();
