...
/* ===== v-final account moderation overrides ===== */
async function sbUpdateProfileByIdOrEmail(profileId, email, payload){
  if(!supabaseClient) return { error: new Error('Supabase yok') };
  let res = await supabaseClient.from('profiles').update(payload).eq('id', profileId);
  if(res.error && email){
    res = await supabaseClient.from('profiles').update(payload).eq('email', email);
  }
  return res;
}

async function sbHandleBlockedProfile(profile, message){
  try{ if(supabaseClient) await supabaseClient.auth.signOut(); }catch(e){ console.error(e); }
  localStorage.removeItem(SESSION_KEY);
  alert(message || (profile?.is_deleted ? 'Bu hesap silinmiş.' : 'Bu hesap banlanmış.'));
  window.location.href = 'login.html';
  return null;
}

const __oldSbEnsureLocalSession = sbEnsureLocalSession;
sbEnsureLocalSession = async function(){
  const sessionUser = await __oldSbEnsureLocalSession();
  if(!sessionUser) return null;
  const authUser = await sbGetUser();
  if(!authUser) return null;
  const profile = await sbGetProfileByUserId(authUser.id);
  if(!profile) return null;
  if(profile.is_deleted){
    return await sbHandleBlockedProfile(profile, 'Bu hesap silinmiş.');
  }
  if(profile.is_banned){
    return await sbHandleBlockedProfile(profile, profile.banned_reason ? ('Bu hesap banlanmış: ' + profile.banned_reason) : 'Bu hesap banlanmış.');
  }
  return sessionUser;
};

const __oldSbRequireRole = sbRequireRole;
sbRequireRole = async function(requiredRole){
  const user = await sbGetUser();
  if(!user){
    localStorage.removeItem(SESSION_KEY);
    window.location.href = 'login.html';
  ...
  if(profile.is_deleted){
    await sbHandleBlockedProfile(profile, 'Bu hesap silinmiş.');
    return false;
  }
  if(profile.is_banned){
    await sbHandleBlockedProfile(profile, profile.banned_reason ? ('Bu hesap banlanmış: ' + profile.banned_reason) : 'Bu hesap banlanmış.');
    return false;
  }
  return await __oldSbRequireRole(requiredRole);
};
...
renderUsersAdmin = async function(){
  ...
  const { data, error } = await supabaseClient
    .from('profiles')
    .select('id,email,full_name,role,is_banned,is_deleted,banned_reason,created_at')
    .order('created_at', { ascending:false });
  if(error){ console.error(error); mount.innerHTML='<div class="status-note">Kullanıcılar yüklenemedi.</div>'; return; }
  if(!data || !data.length){ mount.innerHTML='<div class="status-note">Henüz kullanıcı yok.</div>'; return; }
  const balanceMap = {};
  ...
  mount.innerHTML = data.map(user=>{
    ...
    const statusText = user.is_deleted ? 'Silinmiş' : user.is_banned ? 'Banlı' : 'Aktif';
    const statusClass = user.is_deleted ? 'off' : user.is_banned ? 'warn' : 'ok';
    return `
      <div class="item-row">
        <div>
          <strong>${user.full_name || 'Kullanıcı'}</strong>
          <div class="small">${user.email} • ${user.role === 'admin' ? 'Admin' : 'Müşteri'}</div>
          <div class="small">Bakiye: ${fmtMoney(balanceMap[user.id] || 0)} • <span class="badge ${statusClass}">${statusText}</span></div>
          ${user.banned_reason ? `<div class="small">Sebep: ${user.banned_reason}</div>` : ''}
        </div>
        <div class="item-actions">
          ${(!isSelf && user.role !== 'admin' && !user.is_deleted) ? `
            <button class="small-btn ${user.is_banned ? 'green' : 'gold'}" data-sb-ban-toggle="${user.id}" data-email="${user.email}">${user.is_banned ? 'Ban Kaldır' : 'Banla'}</button>
            <button class="small-btn red" data-sb-soft-delete="${user.id}" data-email="${user.email}">Hesabı Sil</button>
          ` : ''}
        </div>
      </div>`;
  }).join('');

  mount.querySelectorAll('[data-sb-ban-toggle]').forEach(btn=>btn.addEventListener('click', async ()=>{
    const id=btn.getAttribute('data-sb-ban-toggle');
    const email=btn.getAttribute('data-email');
    const row=(data || []).find(x=>x.id===id || x.email===email);
    if(!row) return;
    if(row.is_banned){
      const { error } = await sbUpdateProfileByIdOrEmail(id, email, { is_banned:false, banned_reason:null });
      if(error){ console.error(error); alert('Ban kaldırılamadı.'); return; }
      await renderUsersAdmin();
      return;
    }
    const reason = prompt('Ban sebebi (isteğe bağlı):', row.banned_reason || '') || '';
    const { error } = await sbUpdateProfileByIdOrEmail(id, email, { is_banned:true, banned_reason:reason || null });
    if(error){ console.error(error); alert('Kullanıcı banlanamadı.'); return; }
    await renderUsersAdmin();
  }));

  mount.querySelectorAll('[data-sb-soft-delete]').forEach(btn=>btn.addEventListener('click', async ()=>{
    const id=btn.getAttribute('data-sb-soft-delete');
    const email=btn.getAttribute('data-email');
    if(!confirm('Bu hesap silinsin mi? Bu işlem bu altyapıda yumuşak silme yapar ve girişini engeller.')) return;
    const reason = prompt('Silme notu (isteğe bağlı):', '') || 'Admin tarafından silindi';
    const { error } = await sbUpdateProfileByIdOrEmail(id, email, { is_deleted:true, is_banned:true, banned_reason:reason });
    if(error){ console.error(error); alert('Hesap silinemedi.'); return; }
    await renderUsersAdmin();
  }));
};

window.renderUsersAdmin = renderUsersAdmin;
/* ===== end account moderation overrides ===== */
